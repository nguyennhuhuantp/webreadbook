...
public class ChangePasswordControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String renew = request.getParameter("renewpass");
        DAO dao = new DAO();
        HttpSession session = request.getSession();
        Account a = (Account) session.getAttribute("acc");
        String pass = a.getPass();
        int id = a.getId();
        if (dao.md5(oldp).equals(dao.md5(pass))) {
            if (renew.equals(newp)) {
                dao.changePassword(dao.md5(newp), id + "");
                response.sendRedirect("Login.jsp");
            } else {
                request.setAttribute("mess", "Wrong password to re-enter");
